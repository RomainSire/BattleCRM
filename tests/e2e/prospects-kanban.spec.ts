...
test.describe('Prospects - Kanban Board View', () => {
  ...
  test('default view is list (stage filter buttons visible)', async ({ page }) => {
    // In list mode, funnel stage filter buttons are shown
  ...
  test('clicking "Kanban view" switches to kanban board', async ({ page }) => {
    await page.locator('[aria-label="Kanban view"]').click()
    // Stage filter buttons disappear; column headers appear
    await expect(
  ...
  test('switching back to list view works from kanban', async ({ page }) => {
    ...
    // Stage filter buttons visible again (list mode)
    await expect(page.getByRole('button', { name: 'Lead qualified', exact: true })).toBeVisible()
