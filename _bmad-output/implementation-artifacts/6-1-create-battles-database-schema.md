...
- [x] Task 1: Add `BattleType` to `@battlecrm/shared` (AC: #5)
  - [x] 1.1 Create `packages/shared/src/types/battle.ts` with `BattleType` and `BattleStatus` type
  - [x] 1.2 Export from `packages/shared/src/index.ts`
  - [x] 1.3 Run `pnpm --filter @battlecrm/shared build` — verify no TypeScript errors

- [x] Task 2: Create migration `0013_create_battles_table.ts` (AC: #1, #3, #4)
  - [x] 2.1 Create `apps/backend/database/migrations/0013_create_battles_table.ts`
  - [x] 2.2 Add all columns per the AC schema
  - [x] 2.3 Add partial unique index via raw SQL (standard `table.unique()` does NOT support WHERE clauses — see Dev Notes)
  - [x] 2.4 Add composite index on `(user_id, funnel_stage_id, status)`
  - [x] 2.5 Run `ENV_PATH=../../ node ace migration:run` — verify clean output

- [x] Task 3: Create `Battle` Lucid model (AC: #2)
  - [x] 3.1 Create `apps/backend/app/models/battle.ts`
  - [x] 3.2 Declare all columns with correct TypeScript types
  - [x] 3.3 Add `static forUser = scope(...)` identical to other models
  - [x] 3.4 Add `belongsTo` relations: `User`, `FunnelStage`, `variantA: Positioning`, `variantB: Positioning`, `winner: Positioning` (nullable)
  - [x] 3.5 NO SoftDeletes mixin — battles use hard delete (see Dev Notes)

- [x] Task 4: Write functional schema tests (AC: #1, #2, #3, #4)
  - [x] 4.1 Create `apps/backend/tests/functional/battles/schema.spec.ts`
  - [x] 4.2 Test: can create a battle with all fields
  - [x] 4.3 Test: can create a battle with only required fields (winner_id null, closed_at null)
  - [x] 4.4 Test: `forUser` scope isolates battles between users
  - [x] 4.5 Test: partial unique constraint — two active battles for same (user_id, funnel_stage_id) → DB error
  - [x] 4.6 Test: two active battles for same user but DIFFERENT stages → allowed
  - [x] 4.7 Test: active + closed battles for same (user_id, funnel_stage_id) → allowed (partial index only blocks two `active`)
  - [x] 4.8 Test: `battle_number` is stored correctly
  - [x] 4.9 Run `ENV_PATH=../../ node ace test functional` — all tests pass (0 regressions on 265 existing)
...
- Migration `0013_create_battles_table.ts` created with partial unique index via raw SQL (`CREATE UNIQUE INDEX ... WHERE status = 'active'`) — standard `table.unique()` does not support WHERE clauses.
- `database/schema.ts` auto-regenerated by `node ace migration:run` (Adonis introspection) — Biome format applied post-generation.
- `BattleType` and `BattleStatus` added to `@battlecrm/shared`, exported from index.
- `Battle` model: no SoftDeletes, three `belongsTo` to `Positioning` with explicit `foreignKey` to disambiguate `variantAId`, `variantBId`, `winnerId`.
...
- `packages/shared/src/index.ts` (modified — added battle export)
- `apps/backend/database/migrations/0013_create_battles_table.ts` (created)
